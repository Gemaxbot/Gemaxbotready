...
# Puedes agregar aquí los IDs numéricos exactos de tus grupos si los conoces (ej. -100123456789)
# El bot también reconocerá automáticamente el grupo público por su username 'simplegemax'
ALLOWED_GROUP_IDS = []
...
def webhook():
    try:
        ...
        # Registrar información del grupo en los logs para identificar ID si es necesario
        logging.info(f"Grupo recibido -> Título: '{chat_title}' | ID: {chat_id} | Username: '{chat_username}'")

        # RESTRICCIÓN 2: Permitir solo el grupo con username 'simplegemax' o los IDs configurados en ALLOWED_GROUP_IDS
        is_allowed_group = (chat_username == "simplegemax") or (chat_id in ALLOWED_GROUP_IDS)

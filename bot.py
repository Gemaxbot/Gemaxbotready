...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Usamos modelos alternativos estables si uno presenta congestión masiva
    ...
    for attempt in range(max_retries):
        ...
        for model_name in models_to_try:
            try:
                ...
                error_str = str(e)
                logging.warning(f"Fallo con modelo {model_name} (Intento {attempt+1}): {error_str}")
                continue
        
        if success:
            break
        
        # Espera exponencial progresiva antes del siguiente reintento (4s, 8s, 12s)
        wait_time = 4 * (attempt + 1)
        await asyncio.sleep(wait_time)

    if not reply_text:
        reply_text = "Los servidores de Google están experimentando alta congestión en este momento. Por favor, intenta hacer tu pregunta de nuevo en unos segundos."
